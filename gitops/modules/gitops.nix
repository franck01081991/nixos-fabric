...

{
  options.network-fabric.gitops = {
    enable = lib.mkEnableOption "Enable GitOps for NixOS Fabric";
    ...
    machines = lib.mkOption {
      type = lib.types.attrsOf (lib.types.submodule {
        options = {
          enable = lib.mkEnableOption "Enable GitOps for this machine";
  ...
  config = lib.mkIf config.network-fabric.gitops.enable {
    ...
    system.activationScripts.gitops-init = lib.mkAfter ''
...
EOF
    ...
    system.activationScripts.gitops-external-machines = lib.mkAfter ''
...
EOF
    ...
    environment.sessionVariables.EXTERNAL_MACHINES = lib.concatStringsSep " " (lib.attrNames config.network-fabric.gitops.machines);
  };
}