...
- [Ansible Integration](docs/development/ANSIBLE_INTEGRATION.md) - Ansible workflow
...
This project now includes **deep Ansible integration** through Nix modules for complementary configuration management:

### Key Features

- **Native Nix integration** - Ansible is installed and configured via Nix modules
- **Automatic setup** - Configuration files generated during system activation
- **Automated provisioning** of new nodes
- **Idempotent operations** for repeatable tasks
- **Fabric verification** and health checks
- **Complementary to NixOS** - uses Ansible where it excels

### Quick Start with Ansible

```bash
# Deploy with full Nix+Ansible integration (recommended)
./scripts/deploy-ansible-nix.sh rtr-sapinet

# Traditional NixOS deployment (Ansible disabled)
sudo nixos-rebuild switch --flake .#rtr-sapinet
...
hosts/
└── default-ansible.nix  # Default Ansible configuration
```

### Ansible Runtime Structure

```
/etc/nixos-fabric/ansible/  # Generated by Nix
├── inventory/          # Auto-generated from NixOS hosts
├── playbooks/          # Main playbooks
├── roles/              # Modular roles
│   ├── base/            # Base system setup
│   ├── wireguard/       # WireGuard configuration
│   └── frr/             # FRR routing
├── host_vars/          # Auto-generated from Nix config
├── group_vars/         # Auto-generated from Nix config
└── ansible.cfg         # Auto-generated from Nix config
```

See [Ansible Integration Guide](docs/development/ANSIBLE_INTEGRATION.md) for full details.
